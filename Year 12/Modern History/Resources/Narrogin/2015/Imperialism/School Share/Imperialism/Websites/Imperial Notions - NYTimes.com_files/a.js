...
window.webtrendsAsyncInit=(function(){
    ...
    dcs.init({
        dcsid: "dcsypfq3j00000gsclwfljaeo_6i3w" || "dcsym57yw10000s1s8g0boozt_9t1x",
    ...
    dcs.WT.sourceapp = source_app_map["blogs"] || "blogs"; // source app
    ...
    if(wtHelperFn) {
        ...
        wtHelperFn.setRegistrationSection("U.S.", "Education");
        wtHelperFn.setContentInfo("U.S.", dcs.WT);

        
        // Set interactive information

        var tomMetaTag = "Blog" !== "" ? "Blog" : getMetaTag("tom");
        ...
        // Set imagepages 
        if (/^\/imagepages/.test("/2003/06/04/imperial-notions/")) {
        ...
        if ((tomMetaTag == "Slideshow") || (/^\/slideshow\/[0-9][0-9][0-9][0-9]\/[0-1][0-9]\/[0-3][0-9]\//.test("/2003/06/04/imperial-notions/"))) {
        ...
        if ((/[0-9][0-9][0-9][0-9]\/[0-1][0-9]\/[0-3][0-9]\/sunday-review\//.test("/2003/06/04/imperial-notions/")) || 
             (/[0-9][0-9][0-9][0-9]\/[0-1][0-9]\/[0-3][0-9]\/opinion\/sunday\//.test("/2003/06/04/imperial-notions/"))) {
            dcs.WT.cg_n = "Opinion";
...
window._sf_async_config = {
    ...
    path: "/2003/06/04/imperial-notions/",
    title: window.TimesPeople && TimesPeople.Page && TimesPeople.Page.getTitle() || document.title.replace('- NYTimes.com', '')
};

try {
  window._sf_async_config.sections = ["U.S.", "Education"].join(",");
} catch(e){}

try {
  window._sf_async_config.authors = (["JAVAID KHAN","MICHELLE SALE"]) + "".replace('By ', '').toLowerCase().replace(/\b[a-z]/g, function(){return arguments[0].toUpperCase();});
} catch(e){}
...
if (googleAnalyticsTrackerID["U.S."] !== undefined){
	try {
		var pageTracker = _gat._getTracker(googleAnalyticsTrackerID["U.S."]); 
