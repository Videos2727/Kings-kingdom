...
/**
 * Document ready (remilon code)
 */
$(document).ready(function () {

    //set these for each site, rest of JS should be generic
    var bigAdElement = "div#relatedSchoolsList > ul > li.schoolSummary";
    var bigAdRelativeSchoolNameAnchor = "div.schoolInfoBg > div#schoolSummaryIdentity > div.searchResultsCompanyID > h3 > a";
    var bigAdRelativeSchoolName = bigAdRelativeSchoolNameAnchor + " > span";
    var bigAdAbsoluteSchoolName = bigAdElement + " > " + bigAdRelativeSchoolNameAnchor;
    var bigAdProgramText = bigAdElement
            + " > div.programInfoBg > div.schoolSummaryCategories > div.schoolSummaryDegree > ul.degreeType > li > ul > li > ul > li > a";
    var bigAdRequestInfoButton = bigAdElement + " > div.getStartedBg > div.schoolSummaryGetStarted > div.searchResultsCTA > a";
    var bigAdMenu = bigAdElement
            + " > div.getStartedBg > div.schoolSummaryGetStarted > div.megaMiniForm > form.miniform > div.fields > select.miniformSelect";

    var littleAdElement = "div#relatedSchools > div > ul.schoolList > li.schoolInfo";
    var littleAdRelativeSchoolName = "div.toggleSwitch > em > a";
    var littleAdAbsoluteSchoolName = littleAdElement + " > " + littleAdRelativeSchoolName;
    var littleAdProgramText = littleAdElement + " > ul.programList > li > a";
    var littleAdMenu = littleAdElement + " > div.toggleControlled > span.formContent > div > form.miniform > div.fields > select.miniformSelect";

    var pageViewGuid = guidGenerator();
    if ($("#impressionGuid")) {
        $("#impressionGuid").val(pageViewGuid);
    }
    logLoad(pageViewGuid);

    var searchGuid = $(".searchGuid").eq(0).val();
    ...
    $(bigAdProgramText).click(function () {
        logClick($.trim($(this).text()), $(this).attr("href"));
    });

    $(bigAdRequestInfoButton).click(function () {
        logClick(null, $(this).attr("href"));
    });

    $(bigAdMenu).change(function () {
        //the url is not present in the html element but right now always goes to form, so hard-code it
        logClick($(this).find("option:selected").text(), "/form/school.htmlformId=always")
    });

    $(bigAdAbsoluteSchoolName).click(function () {
        logClick($.trim($(this).text()), $(this).attr("href"));
    });
    ...
    $(littleAdProgramText).click(function () {
        logClick($.trim($(this).text()), $(this).attr("href"));
    });

    $(littleAdMenu).change(function () {
        //the url is not present in the html element but right now always goes to form, so hard-code it
        logClick($(this).find("option:selected").text(), "/form/school.html?formId=always")
    });

    $(littleAdAbsoluteSchoolName).click(function () {
        logClick($.trim($(this).text()), $(this).attr("href"));
    });
